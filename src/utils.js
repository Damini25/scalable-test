...
const calculateTimeSeries = ({ years, mu, sigma, fee, initialSum, monthlySum }) => {
    ...
    for (let k=0; k<series.length; k++) {
        allSeries.median.push({ y: series[k].median.toFixed(2), x: series[k].x });
        allSeries.upper95.push({ y: series[k].upper95.toFixed(2), x: series[k].x });
        allSeries.lower05.push({ y: series[k].lower05.toFixed(2), x: series[k].x });
    }
